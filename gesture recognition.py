...
minV = vRange[0]
maxV = vRange[1]
media = vlc.MediaPlayer("EaseUS download video edited.mp4")
media.play()
...
with mp_hands.Hands(
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            x11, y11 = int(hand_landmarks.landmark[mp_hands.HandLandmark.PINKY_TIP].x * wCam), int(
                hand_landmarks.landmark[mp_hands.HandLandmark.PINKY_TIP].y * hCam)
            x10, y10 = int(hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP].x * wCam), int(
            ...
            if (x_roi1 < x10) and (x11 < x_roi2):
                ...
                x3, y3 = int(hand_landmarks.landmark[mp_hands.HandLandmark.WRIST].x * wCam), int(
                    hand_landmarks.landmark[mp_hands.HandLandmark.WRIST].y * hCam)
                x4, y4 = int(hand_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_TIP].x * wCam), int(
                ...
                x1, y1 = int(hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP].x * wCam), int(
                    hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP].y * hCam)
                x2, y2 = int(hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP].x * wCam), int(
